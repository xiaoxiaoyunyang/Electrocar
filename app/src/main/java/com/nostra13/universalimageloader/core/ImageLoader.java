        ...
        if (bmp != null && !bmp.isRecycled()) {
            ...
            if (SysModel.imageLoaderType == 1) {
                ...
                if (imageFile != null && imageFile.exists() && imageFile.length() > 0) {
                    try {
                        bmp = displayTask.decodeImage(ImageDownloader.Scheme.FILE.wrap(imageFile.getAbsolutePath()));
                        if (bmp != null && options.isCacheInMemory()) {
                            LogUtils.logE("添加内存缓存");
                            LogUtils.logE("memoryCacheKey  " + memoryCacheKey);
                ...
                if (bmp != null && !bmp.isRecycled()) {
                    LogUtils.logE("get from sdDir");
                    L.d(LOG_LOAD_IMAGE_FROM_MEMORY_CACHE, memoryCacheKey);

                    if (options.shouldPostProcess()) {
                        ProcessAndDisplayImageTask processDisplayTask = new ProcessAndDisplayImageTask(engine, bmp, imageLoadingInfo,
                                defineHandler(options));
                        if (options.isSyncLoading()) {
                            processDisplayTask.run();
                        } else {
                            engine.submit(processDisplayTask);
                        }
                    } else {
                        options.getDisplayer().display(bmp, imageAware, LoadedFrom.DISC_CACHE);
                        listener.onLoadingComplete(uri, imageAware.getWrappedView(), bmp);
                    }
                } else {
                    ...
                    LogUtils.logE("get from network");
                    //在缓存中没有图片在显示加载过程图片  -------------------------
                    listener.onLoadingStarted(uri, imageAware.getWrappedView());
                    if (options.isSyncLoading()) {
                        displayTask.run();
