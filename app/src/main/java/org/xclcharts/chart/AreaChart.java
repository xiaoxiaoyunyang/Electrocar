...
public class AreaChart extends LnChart {
		...
		if (count == 3) {
			...
			if (areaData.getApplayGradient()) {
				...
				if (areaData.getGradientDirection() == XEnum.Direction.VERTICAL) {
                    ...
                    //新增
                    if(areaData.getApplayGradientWay()){
		...
		if (areaData.getApplayGradient()) {
			...
			if (areaData.getGradientDirection() == XEnum.Direction.VERTICAL) {
                ...
                if(areaData.getApplayGradientWay()){
                    linearGradient = new LinearGradient(0, 0, 0, lineMaxY,
                            areaData.getColors(),
                            areaData.getPositions(), areaData.getGradientMode());
                }else {
                    linearGradient = new LinearGradient(0, 0, 0, lineMaxY,
                            areaData.getAreaBeginColor(),
	...
	private boolean renderLine(Canvas canvas, AreaData areaData,
		...
		for (int i = 0; i < count; i++) {
            ...
            if(areaData.getLineGradient()){
                if(i < count / 3){
                    areaData.getLinePaint().setColor(areaData.getLineBeginColor());
                }else{
                    areaData.getLinePaint().setColor(areaData.getLineEndColor());
                }
            }else{
                areaData.getLinePaint().setColor(areaData.getLineColor());
            }
