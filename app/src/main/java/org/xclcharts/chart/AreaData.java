...
public class AreaData extends LineData {
    ...
    //渲染构造函数实现的方式
    private boolean mApplayGradientWay = false;
		
	private int mAreaBeginColor = Color.WHITE;
    private int mAreaEndColor = Color.WHITE;
    private int mLineBeginColor = Color.WHITE;
    private int mLineEndColor = Color.WHITE;
    //新增
    private int[] colors = {Color.WHITE,Color.WHITE,Color.WHITE};
    private float[] positions = {0.4f,0.8f,1.0f};
     ...
     * @param dataSeries
     * @param lineColor
     * @param areaBeginColor
     * @param colors
     */
    public AreaData(String key,List<Double> dataSeries,int lineColor,
                    int areaBeginColor,int[] colors,float[] positions)
    {
        setLabel(key);
        setDataSet(dataSeries);
        setLineColor(lineColor);
     ...
     * @param key
     * @param dataSeries
     * @param lineStartColor
     * @param lineEndColor
     * @param areaBeginColor
     * @param colors
     * @param positions
     */
    public AreaData(String key,List<Double> dataSeries,int lineStartColor,int lineEndColor,
                    int areaBeginColor,int[] colors,float[] positions)
    {
        setLabel(key);
        setDataSet(dataSeries);
        setLineColor(lineStartColor);
        getDotPaint().setColor(Color.TRANSPARENT);
        setLineBeginColor(lineStartColor);
        setLineEndColor(lineEndColor);

        setLineGradient(true);
        setAreaFillColor(areaBeginColor);
    ...
    /**设置 渲染构造实现方式
     *
     * @param mApplayGradientWay 状态
     */
    public void setApplayGradientWay(boolean mApplayGradientWay) {
        this.mApplayGradientWay = mApplayGradientWay;
    }
     ...
     * @return
     */
    public boolean getApplayGradientWay(){
        if(getColors() == null)
            return false;
        return mApplayGradientWay;
    }
    /**
	 * 设置渐变渲染方向	
	 * @param direction 方向
    ...
    public void setLineBeginColor(int mLineBeginColor) {
        this.mLineBeginColor = mLineBeginColor;
    }

    public void setLineEndColor(int mLineEndColor) {
        this.mLineEndColor = mLineEndColor;
    }

    public int getLineBeginColor() {
        return mLineBeginColor;
    }
